...
public class Mastermind<T> {
	...
	/**
	 * Create a new Mastermind instance with a random solution out of the given
	 * pool with the defined size.
	 * 
	 * @param size
	 *            The size of the solution - How many elements may a solution
	 *            have (this would be 4 in the original game)?
	 * @param options
	 *            The pool of options from which the solution should be created.
	 *            (This would be 6 colors in the original game)
	 * @return A new Mastermind instance with a random solution based on the
	 *         given parameters.
	 */
	@SafeVarargs
	 ...
	 * @return A result containing the correct hits (color at the correct
	 *         position) and the "color" hits (right color at the wrong
	 *         position)
	 */
	 ...
	 */
	private static <T> int checkForColorHits(List<T> guessCopy,
			List<T> solutionCopy) {
		int hits = 0;
	...
	/**
	 * Traverses the given List copies for guess and solution from last to
	 * first.
	 * <p>
	 ...
	 */
	private static <T> int checkForDirectHits(List<T> guessCopy,
			List<T> solutionCopy) {
		int hits = 0;
